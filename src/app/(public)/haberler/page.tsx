...
export async function generateMetadata({
  ...
  const label = params.label
    ? `${params.label} Haberleri`
    : "Eğitim Gündemi ve Haberler";

  return {
    title: `${label} | Büşra Öğretmen`,
    description:
      "Öğretmen atamaları, MEB duyuruları, maaş zamları ve eğitim dünyasındaki son dakika gelişmeleri.",
  };
}
    ...
    <div className="max-w-6xl mx-auto pb-12">
      <header className="mb-10 text-center md:text-left">
        ...
        <p className="text-lg text-slate-500 max-w-2xl">
          Eğitim dünyasındaki en son gelişmeleri, MEB duyurularını ve atama
          haberlerini buradan takip edin.
        </p>
      </header>
...
async function NewsList({
  ...
  /**
   * Public haber sayfasında yalnızca yayınlanmış haberler.
   *
   * Bu yalnızca SELECT sorgusunu filtreler.
   * Veritabanında hiçbir kayıt değişmez/silinmez.
   */
    ...
    return (
      <div className="flex flex-col items-center justify-center bg-white border border-slate-100 rounded-3xl p-16 text-center shadow-sm">
        ...
        <p className="text-slate-500 max-w-md">
          Arama kriterlerinize veya seçtiğiniz kategoriye ait henüz bir haber
          yayınlanmamış.
        </p>
      </div>
    );
