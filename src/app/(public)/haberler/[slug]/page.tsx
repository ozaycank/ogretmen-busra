...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  // TS UYUMU İÇİN DÜZELTİLDİ: findUnique yerine findFirst kullanıyoruz. 
  const news = await prisma.news.findFirst({ 
    where: isId ? { id: identifier, status: PostStatus.PUBLISHED } : { slug: identifier, status: PostStatus.PUBLISHED } 
  });
  ...
  return {
    ...
    alternates: { canonical: `/haberler/${news.slug}` },
    openGraph: {
      title: news.title,
      description: news.content.substring(0, 160) + "...",
      type: "article",
      url: `/haberler/${news.slug}`,
...
export default async function NewsDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // ZORUNLU GÜVENLİK (PUBLISHED kontrolü) ve TS UYUMU (findFirst)
  const news = await prisma.news.findFirst({
    where: isId ? { id: identifier, status: PostStatus.PUBLISHED } : { slug: identifier, status: PostStatus.PUBLISHED },
  });
  ...
  const shareUrl = `${siteUrl}/haberler/${news.slug}`;
  ...
  const jsonLd = {
    ...
    "author": [{ "@type": "Person", "name": "Büşra Öğretmen Editör Ekibi", "url": `${siteUrl}/hakkimizda` }]
  };
      ...
      <div className="max-w-4xl mx-auto pb-12 space-y-10">
        ...
        <header className="space-y-6">
          <div className="flex flex-wrap items-center gap-3">
            <span className="bg-rose-50 text-rose-600 px-4 py-1.5 rounded-full text-sm font-bold tracking-wide">{news.label}</span>
          </div>
          <h1 className="text-3xl md:text-5xl font-black text-slate-900 leading-tight">{news.title}</h1>
          <div className="flex flex-wrap items-center gap-6 text-sm text-slate-500 py-4 border-y border-slate-100">
            <div className="flex items-center gap-2"><Calendar size={18} className="text-slate-400"/>{new Date(news.createdAt).toLocaleDateString("tr-TR", { year: 'numeric', month: 'long', day: 'numeric' })}</div>
            <div className="flex items-center gap-2"><Clock size={18} className="text-slate-400"/>{readingTime} Dk Okuma</div>
            <div className="flex items-center gap-2"><Eye size={18} className="text-slate-400"/>{news.viewCount + 1} Okunma</div>
          </div>
        </header>
        {news.imageUrl && (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100 mt-12">
          <div className="flex items-center gap-2 text-slate-900 font-bold"><Share2 size={20} className="text-slate-500" />Haberi Paylaş:</div>
        ...
        <section className="pt-12 border-t border-slate-100">
          <div className="flex justify-between items-end mb-8">
            ...
            <Link href="/haberler" className="text-sky-600 font-bold hover:underline hidden sm:block">Tümüne Git &rarr;</Link>
          </div>
          <Suspense fallback={<div className="grid grid-cols-1 md:grid-cols-2 gap-6"><SkeletonCard /><SkeletonCard /></div>}>
...
async function RelatedNews({ label, currentId }: { label: string, currentId: string }) {
  const relatedNews = await prisma.news.findMany({
    where: { label: label, id: { not: currentId }, status: PostStatus.PUBLISHED },
