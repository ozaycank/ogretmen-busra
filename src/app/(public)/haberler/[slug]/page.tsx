...
import { renderNewsContent } from "@/modules/news/utils/news-content.server";
...
export async function generateMetadata({
  ...
  const description =
    news.seoDescription?.trim() || createNewsExcerpt(news.content, 160);

  return {
    title: `${news.seoTitle?.trim() || news.title} | Büşra Öğretmen`,

    description,

    alternates: {
      canonical: `/haberler/${news.slug}`,
    },
...
export default async function NewsDetailPage({
  ...
  /**
   * Eski UUID linkleri SEO slug'a yönlendirmeye devam eder.
   */
  if (isId && news.slug) {
  ...
  /**
   * Mevcut viewCount davranışını koruyoruz.
   * Haber içeriğine veya başka DB kayıtlarına dokunmaz.
   */
  prisma.news
    .update({
      where: {
  ...
  /**
   * Yeni Quill HTML -> sanitize edilmiş HTML
   * Eski Markdown -> HTML -> sanitize edilmiş HTML
   *
   * Veritabanı burada değiştirilmez.
   */
  const renderedContent = renderNewsContent(news.content);

  const siteUrl =
    process.env.NEXT_PUBLIC_APP_URL || "https://www.ogretmenbusra.com";
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto pb-12 space-y-10">
        <nav className="flex items-center text-sm font-medium text-slate-500 overflow-x-auto whitespace-nowrap pb-2">
          ...
          <Link
            href={`/haberler?label=${encodeURIComponent(news.label)}`}
            className="hover:text-sky-600 transition-colors"
          >
            {news.label}
          </Link>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100 mt-12">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="p-3 bg-white hover:bg-[#1877F2] hover:text-white text-slate-400 rounded-full shadow-sm transition-all"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-3 bg-white hover:bg-[#1DA1F2] hover:text-white text-slate-400 rounded-full shadow-sm transition-all"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-3 bg-white hover:bg-[#0A66C2] hover:text-white text-slate-400 rounded-full shadow-sm transition-all"
          ...
          <div className="flex justify-between items-end mb-8">
            <div>
              ...
              <p className="text-slate-500 mt-1">
                Bu konudaki diğer gelişmelere göz atın.
              </p>
            </div>

            <Link
              href="/haberler"
              className="text-sky-600 font-bold hover:underline hidden sm:block"
            >
              Tümüne Git &rarr;
            </Link>
          </div>
